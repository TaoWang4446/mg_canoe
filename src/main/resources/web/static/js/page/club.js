...
function change(name,id) {
    var btn = $("<a title='修改' href='/setting/club/detail/"+id+"' class='layui-btn margin_0 change_info'>修改</a>");
...
function delStatus(id) {
    layer.confirm('请确认是否删除', {btn: ['确认', '取消'], area: '500px'},function () {
        $.ajax({
            url: "/setting/deleteClub/"+id,
            type: "POST",
            success: function (result) {
                if(result.status==200){
                    layer.msg('删除成功!', {icon: 1, time: 1000})
                }else {
                    layer.msg('此俱乐部下还有成员，不可删除!', {icon: 1, time: 1000})
                }
                search.ajax.reload();
